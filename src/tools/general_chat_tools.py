...
def get_currency_exchange(from_currency: str, to_currency: str, amount: float = 1.0) -> dict:
    ...
    Returns the exchange rate, converted amount, and the data source timestamp.
    """
...
# Simplified matrix: {passport: {destination: visa_info}}
# Covers common passport/destination combos used by the target user base.
...
def get_visa_requirements(passport_nationality: str, destination: str) -> dict:
    ...
    passport_nationality: the user's passport country (e.g. 'Israeli', 'American', 'British', 'French').
    destination: the destination country or city (e.g. 'Japan', 'Tokyo', 'Thailand').
    """
    dest = validate_city(destination)

    # Try to match nationality key (case-insensitive)
    nat_key = None
    for key in _VISA_DB:
...
def _infer_climate(season: str, destination: str) -> str:
    """Rough climate inference from season and known hot/cold regions."""
...
def get_packing_list(destination: str, season: str, trip_days: int, trip_type: str = "city") -> dict:
    ...
    # Scale clothing quantities for longer trips
    if trip_days > 10:
...
def get_local_customs(destination: str) -> dict:
    ...
    - 'What should I know before going to [destination]?'
    """
    dest = validate_city(destination)

    # Try direct match first, then country-level fallback
