...
from agent.nodes.itinerary.itinerary_tools import (
    search_activities, get_weather, calculate_trip_cost, get_average_location_cost,
)
...
class ItineraryExecutorNode:
    ...
    def _run_build_day(
        ...
        if mode == "with_travel_data":
            cfg = self._build_config_from_travel_plan(state, day_num, trip_days, candidates)
        else:
            cfg = self._build_config_standalone(state, day_num, trip_days, candidates)
