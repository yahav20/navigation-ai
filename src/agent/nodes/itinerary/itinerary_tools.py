...
USAGE: The Executor picks the right tool per step. All tools return 
JSON-serializable dicts/lists or raise ToolException on failure.
"""
...
def fetch_flights(origin: str, destination: str) -> list[dict]:
    ...
    try:
        ...
        raise ToolException(f"fetch_flights failed: {e}")


@tool
def fetch_return_flights(origin: str, destination: str) -> list[dict]:
    """
    ...
    try:
        ...
        raise ToolException(f"fetch_return_flights failed: {e}")
...
def fetch_hotels(
    ...
    try:
        ...
        raise ToolException(f"fetch_hotels failed: {e}")
...
def fetch_activities(
    ...
    try:
        ...
        # Sorting by price as a fallback since rating isn't in the provided DB schema
        return sorted(activities, key=lambda a: a.get("price", 0))
    except Exception as e:
        raise ToolException(f"fetch_activities failed: {e}")
...
def fetch_weather(city: str) -> dict:
    ...
    try:
        ...
        raise ToolException(f"fetch_weather failed: {e}")
...
def verify_budget(
    ...
    This corresponds directly to the 'verify_budget' step from the Planner.
    """
    try:
        ...
        raise ToolException(f"verify_budget failed: {e}")
