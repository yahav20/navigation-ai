...
def _render_header(plan: dict) -> str:
    ...
    # Solo baseline — omit when traveling alone (default case, no extra info)
    solo = plan.get("lowest_total_estimate")
    label = plan.get("travelers_label") or "1 adult"
    if solo is not None and label != "1 adult":
        lines.append(f"**Lowest Estimate (if traveling solo):** ${float(solo):.0f}")

    # Group total — show whenever computed (includes solo=1 adult for consistency)
    group = plan.get("lowest_group_estimate")
    if group is not None:
        lines.append(f"**Group Total Estimate:** ${float(group):.0f} for {label}")
